...
class Enqueue extends Engine\Base {
	...
	public function enqueue_styles__premium_only() {
		...
		if ( ( $this->is_shortcode_in_page( 'glossary-list' ) || \has_block( 'glossary/list' ) ) ||
			\is_tax( 'glossary-cat' ) &&
	...
	public function enqueue_css_customizer__premium_only() {
		...
		if (
			\is_array( $custom_css ) && (
				!isset( $custom_css[ 'on_mobile' ] ) ||
				'responsive' !== $custom_css[ 'on_mobile' ]
			)
		) {
			return false;
		}
	...
	/**
	 * Register and enqueues public-facing JavaScript files on premium.
	 *
	 * @return void
	 */
	public function enqueue_scripts__premium_only() {
		$custom_css = \get_option( GT_SETTINGS . '-customizer' );
		$this->enqueue_shortcode_scripts__premium_only();
	...
	/**
	 * Register and enqueues shortcode assets.
	 *
	 * @return void
	 */
	public function enqueue_shortcode_scripts__premium_only() {
		global $post;
		...
		if ( \strpos( $post->post_content, 'accordion=' ) === false ) {
			return;
		}

		\wp_enqueue_script(
			GT_SETTINGS . '-a2z-accordion',
			\plugins_url( 'assets/js/a2z-accordion.js', GT_PLUGIN_ABSOLUTE ),
			array(),
			GT_VERSION,
			true
		);
