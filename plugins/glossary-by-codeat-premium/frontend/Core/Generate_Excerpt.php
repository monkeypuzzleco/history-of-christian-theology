...
class Generate_Excerpt extends Engine\Base {
	 ...
	 * @return string
	 */
	public function get_the_excerpt() { //phpcs:ignore SlevomatCodingStandard.Complexity.Cognitive.ComplexityTooHigh
		$excerpt = '';
	...
	public function elaborate_the_excerpt( string $excerpt ) {
		...
		$excerpt = \str_replace( array( '<b...', '<...', '<br...', '<br>...' ), '...', $excerpt );
	...
	public function noreadmore( array $atts ) {
		...
		// Remove the More link setting
		if ( \gt_fs()->is_plan__premium_only( 'professional' ) ) {
			if ( !empty( $this->settings[ 'more_link' ] ) ) {
				$readmore = '<span class="' . $this->default_parameters[ 'css_class_prefix' ] . '-more-mobile">' . $readmore . '</span>';
			}
		}
