...
class Term_Injector extends Engine\Base {
	 ...
	 * @return string
	 */
	public function do_wrap( string $text, array $terms ) { //phpcs:ignore SlevomatCodingStandard.Complexity.Cognitive.ComplexityTooHigh
	...
	public function replace_with_utf_8() {
		...
		foreach ( $this->terms_to_inject as $original_position => $term ) {
			...
			$new_text = \substr_replace( $new_text, $term_value, $insert_position, $this->get_real_length( $value, $original_term_length ) );
		}
