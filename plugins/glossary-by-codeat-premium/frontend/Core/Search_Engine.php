...
class Search_Engine extends Engine\Base {
	...
	public function initialize() {
		...
		if ( \function_exists( 'et_setup_theme' ) ) {
			\add_action( 'do_shortcode_tag', array( $this, 'check_divi_block' ), 3, 9999 );
		}
		...
		if ( \apply_filters( $this->default_parameters[ 'filter_prefix' ] . '_excerpt_support', true ) ) {
	 ...
	 * @return string
	 */
	public function check_auto_link( $text ) { // phpcs:ignore
		$text = \strval( $text );
	 ...
	 * @return string
	 */
	public function auto_link( $text ) { // phpcs:ignore
		if ( empty( $this->terms ) ) {
	...
	public function check_divi_block( $output, $tag, $attr ) {
		if (
			(
				( isset( $attr[ 'theme_builder_area' ] ) && $attr[ 'theme_builder_area' ] === 'post_content' )
				|| isset( $attr[ '_builder_version' ] )
			)
			&& \in_array( $tag, array( 'et_pb_text', 'et_pb_wc_description' ), true )
		) {
