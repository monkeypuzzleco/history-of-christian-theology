...
function get_glossary_terms_list( string $order, int $num, string $tax = '', string $theme = '' ) {
	...
	if ( !in_array( $theme, array( 'hyphen', 'tilde', 'dot', 'arrow' ), true ) ) {
		$theme = '';
	}
 ...
 * @return string
 */
function get_glossary_cats_list_li( WP_Term $tax, array $taxs ) { // phpcs:ignore
	...
	foreach ( $taxs as &$subcategory ) { //phpcs:ignore
 ...
 * @return void
 */
function gl_update_counter() { //phpcs:ignore SlevomatCodingStandard.Complexity.Cognitive.ComplexityTooHigh
	$args  = array(
		'post_type'      => 'glossary',
		'posts_per_page' => -1,
		'order'          => 'asc',
...
function gl_get_a2z_ids( array $atts = array() ) {
	...
	if ( false === $query_output ) {
		$query_output = $wpdb->get_results( "SELECT ID FROM {$wpdb->posts}" . $sanitized_sql_pieces[ 'join_tables' ] . " WHERE {$wpdb->posts}.post_status = 'publish' AND {$wpdb->posts}.post_type = 'glossary'" . $sanitized_sql_pieces[ 'tax_slug' ] . $sanitized_sql_pieces[ 'initials' ] . " ORDER BY TRIM( UPPER( {$wpdb->posts}.post_title ) );", ARRAY_A );  //phpcs:ignore
		wp_cache_set( $cache_key, $query_output );
	}
