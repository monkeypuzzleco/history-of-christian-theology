...
class A2Z extends \WPH_Widget {
		...
		if ( \gt_fs()->is_plan__premium_only( 'professional' ) ) {
			$args[ 'fields' ][] = array(
				...
				'fields' => array(
					...
					array(
						'name'  => \__( 'Underlined', GT_TEXTDOMAIN ),
						'value' => 'underlined',
					),
					array(
						'name'  => \__( 'Tiled', GT_TEXTDOMAIN ),
						'value' => 'tiled',
					),
				),
			);
	...
	public function generate_list_item( array $instance ) {
		...
		$pt_initials = \gl_get_a2z_initial( array( 'show_counts' => $show_counts, 'taxonomy' => $instance['tax' ] ) );
		$initial_arr = array();
