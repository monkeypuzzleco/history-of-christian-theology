...
class Notices extends Engine\Base {
	...
	public function load_notices() {
		...
		if ( \defined( 'ACF' ) ) {
			$this->dismissable(
				\__(
					'This website has the Advanced Custom Fields plugin that is <a href="https://docs.codeat.co/glossary/faq/#can-i-use-glossary-with-acf">supported by the Glossary plugin</a>.',
			GT_TEXTDOMAIN
				),
				'_pro_acf',
				'updated'
		...
		if ( \is_array( $this->settings ) && !isset( $this->settings[ 'archive' ] ) ) {
			if ( isset( $this->settings[ 'slug' ] ) &&
					!\is_null( \get_page_by_path( $this->settings[ 'slug' ], OBJECT ) ) ) {
	...
	public function alerts_by_libraries() {
		...
		if ( $builder->has_entropy() ) {
			...
			if ( \gt_fs()->is_plan__premium_only( 'professional' ) ) {
				$alert = \__(
						'We have discovered a page builder on your website that may cause issues if used in conjunction with the glossary. Don\'t forget to check our <a href="https://docs.codeat.co/glossary/faq/#do-you-support-visual-composer" target="_blank">documentation</a> before submitting any support requests.',
						GT_TEXTDOMAIN
					);
			}
	...
	public function dismissable( $message, $key, $type ) {
		...
		new PermanentDismissibleNotice( $message, GT_TEXTDOMAIN . $key, $type, 10, array(), true );
	}
	...
	public function get_post() {
		...
		$post_id = \intval( \wp_unslash( $_GET['post'] ) ); // phpcs:ignore
	...
	public function content_excerpt_empty() {
		...
		if (
			...
			|| !empty( \trim( $post->post_content ) )
			|| !empty( \trim( $post->post_excerpt ) )
		) {
			return false;
		}
	...
	public function content_has_list_shortcode() {
		...
		new Notice(
			\sprintf(
				...
				\__(
					'The Glossary shortcode list is used in this page and has a daily cache, you need to <a href="%s" target="_blank">manually purge it</a> to see new terms without waiting!',
					GT_TEXTDOMAIN
				),
