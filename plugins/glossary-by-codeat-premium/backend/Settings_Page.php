...
class Settings_Page extends Engine\Base {
	...
	public function initialize() {
		...
		if ( \gt_fs()->is_plan__premium_only( 'professional' ) ) {
			if ( \current_user_can( 'manage_options' ) &&
				\is_admin() && isset( $_GET[ 'gt_preview' ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				\add_action( 'init', array( $this, 'preview__premium_only' ) );
			}
		}
	 ...
	 * @return void
	 */
	public function preview__premium_only() {
	...
	public function add_action_links( array $links ) {
		return \array_merge(
			array(
				'settings' => '<a href="' . \admin_url( 'edit.php?post_type=glossary' ) . '">' . \__(
					'Settings',
					GT_TEXTDOMAIN
					) . '</a>',
			),
	...
	public function purge_transients() {
		if ( empty( $_GET[ 'gl_purge_transient' ] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
			return;
		}
		...
		// Execute our query
		$transients = $wpdb->get_results( $wpdb->prepare( $sql, $prefix . '%' ), ARRAY_A ); //phpcs:ignore WordPress.DB
