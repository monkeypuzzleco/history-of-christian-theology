 ...
 *   ctl_create_admin_notice( array('id'=>'bp-greeting-mesage','message'=>'Hey there!') );
 *
 *              create a admin text error message
 * ctl_create_admin_notice( array('id'=>'bp-error-mesage','message'=>'this is an example of error!','type'=>'error') );
...
if ( ! class_exists( 'ctl_admin_notices' ) ) :
	...
	final class ctl_admin_notices {
		...
		private function ctl_simple_notice( $id, $message ) {
			...
			if ( $img_path != null ) {
				$image_html = '<div class="logo_container"><a href="https://wordpress.org/plugins/timeline-widget-addon-for-elementor/"><img src="' . $img_path . '" style="max-width:70px;"></a></div>';
			} else {
				$image_html = '';
			}
			echo "<div class='" . $id . "_admin_notice $classes ctl-simple-notice' data-ajax-url='" . admin_url( 'admin-ajax.php' ) . "' data-wp-nonce='" . $nonce . "' data-plugin-slug='$id'>$image_html<div class='message_container'><p>" . $message['message'] . '</p></div></div>' . $script;
		}
		...
		private function ctl_admin_notice_for_review( $id, $messageObj ) {
			...
			if ( get_option( 'cool-timelne-pro-installDate' ) ) {
				...
				// $installation_date =date( 'Y-m-d h:i:s', get_option( 'ctl-installDate' ));
				$installation_date = date( 'Y-m-d h:i:s', strtotime( get_option( 'cool-timelne-pro-installDate' ) ) );
			} else {
				// $this->ctl_show_error('Review notice can not be integrated. ctl-installDate option is not set for the plugin');
				return;
			}
		...
		function ctl_create_notice_content( $id, $messageObj ) {
			...
			$message      = "Thanks for using <b>$plugin_name</b> WordPress plugin. We hope it meets your expectations! Please share a few minutes from your valuable time to add a review on <a href='$plugin_link' target='_blank'><strong>codecanyon</strong><a/> or <a href='$wp_link' target='_blank'><strong>WP.org</strong></a>, it works as a boost for us to keep working on more <a href='https://coolplugins.net' target='_blank'><strong>cool plugins</strong></a>!<br/>";
		...
		public function ctl_admin_review_notice_dismiss() {
			...
			if ( isset( $_REQUEST['_nonce'] ) && ! empty( $_REQUEST['_nonce'] ) && wp_verify_nonce( $_REQUEST['_nonce'], $nonce_key ) ) {
				$rs = update_option( 'cool-timeline-pro-already-rated', 'yes' );
