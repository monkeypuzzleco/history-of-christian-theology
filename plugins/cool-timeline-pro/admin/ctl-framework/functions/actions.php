...
if ( ! function_exists( 'ctl_get_icons' ) ) {
  function ctl_get_icons() {
    ...
    if ( ! wp_verify_nonce( $nonce, 'ctl_icon_nonce' ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid nonce verification.', 'cool-timeline' ) ) );
    }
    ...
    if ( ! empty( $icon_lists ) ) {
      ...
      echo '<div class="ctl-error-text">'. esc_html__( 'No data available.', 'cool-timeline' ) .'</div>';
...
if ( ! function_exists( 'ctl_export' ) ) {
  function ctl_export() {
    ...
    if ( ! wp_verify_nonce( $nonce, 'ctl_backup_nonce' ) ) {
      die( esc_html__( 'Error: Invalid nonce verification.', 'cool-timeline' ) );
    }

    if ( empty( $unique ) ) {
      die( esc_html__( 'Error: Invalid key.', 'cool-timeline' ) );
    }
...
if ( ! function_exists( 'ctl_import_ajax' ) ) {
  function ctl_import_ajax() {
    ...
    if ( empty( $unique ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid key.', 'cool-timeline' ) ) );
    }

    if ( empty( $data ) || ! is_array( $data ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: The response is not a valid JSON response.', 'cool-timeline' ) ) );
    }
...
if ( ! function_exists( 'ctl_chosen_ajax' ) ) {
  function ctl_chosen_ajax() {
    ...
    if ( empty( $type ) || empty( $term ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid term ID.', 'cool-timeline' ) ) );
    }

    $capability = apply_filters( 'ctl_chosen_ajax_capability', 'manage_options' );

    if ( ! current_user_can( $capability ) ) {
      wp_send_json_error( array( 'error' => esc_html__( 'Error: You do not have permission to do that.', 'cool-timeline' ) ) );
    }
