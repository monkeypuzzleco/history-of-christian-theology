...
if ( ! class_exists( 'CTL_Shortcode_Preivew' ) ) {
	...
	class CTL_Shortcode_Preivew {
		...
		public function ctl_preview_custom_css() {
			$style = '.ctl-wrapper {max-width: calc(100% - 200px) !important}
			.ctl-wrapper .ctp-media-slider img{width: 100% !important}
			.ctl-wrapper .ctl-slider-wrapper,.ctl-wrapper .ctl-story,ctl_load_more_pagination,.ctl-category-container,.ctl-navigation-bar{pointer-events: none}';
			return $style;
		}
		...
		public function ctl_preview_css() {
			$file_names = array( 'swiper.min.css', 'ctl-common-styles.min.css' );
		...
		public function ctl_preview_script() {
			$file_names = array( 'swiper.min.js', 'ctl-common.js' );
			if ( 'horizontal' === $this->designs ) {
				array_push( $file_names, 'ctl-horizontal.js' );
			} elseif ( 'compact' === $this->designs ) {
				array_push( $file_names, 'ctl-vertical.js' );
				array_push( $file_names, 'masonry.pkgd.min.js' );
				array_push( $file_names, 'imagesloaded.pkgd.min.js' );
			} else {
				array_push( $file_names, 'ctl-vertical.js' );
			}
		...
		public function files_path( $files, $type ) {
			...
			foreach ( $files as $file ) {
				$file_path = CTP_PLUGIN_URL . $dir . $file;
