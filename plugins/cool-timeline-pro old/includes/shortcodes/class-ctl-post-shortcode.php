 ...
 */
if ( ! class_exists( 'CTL_Post_Shortcode' ) ) {

	/**
	 * Class Shortcode.
	 */
	class CTL_Post_Shortcode {
		...
		public function ctl_layout_loader() {
			...
			require_once CTP_PLUGIN_DIR . 'includes/shortcodes/class-ctl-post-loop-helper.php';
		...
		public function ctl_register_shortcode() {
			add_shortcode( 'cool-content-timeline', array( $this, 'ctl_post_shortcode_handler' ) );
		 ...
		 */
		public function ctl_post_shortcode_handler( $atts, $content = null ) {
			/**
			...
			$this->attributes['ctl_type'] = 'post_timeline';

			// Shorticode animation attribute migration with animations attribute.
			$this->attributes['animation'] = array_key_exists( 'animations', $atts ) ? $atts['animations'] : $this->attributes['animation'];
			...
			$wp_query                  = CTL_Query_Builder::post_get_query( $this->attributes, $this->settings );
		 ...
		 * @param array $atts shortcode attributes.
		 */
		public function setting_migration( $atts ): void {
			...
			$old_attributes = array(
				...
				'post-meta'          => in_array( $this->settings['display_post_meta'], array( 'yes', 'Yes' ), true ) ? 'show' : 'hide',
				...
				'pagination'         => array_key_exists( 'pagination', $atts ) && 'ajax_load_more' === $atts['pagination'] ? 'ajax_load_more' : $default_pagination,
				'icons'              => array_key_exists( 'icons', $atts ) && 'none' === $atts['icons'] ? 'none' : ( array_key_exists( 'icons', $atts ) && in_array( $atts['icons'], $valid_icons, true ) ? 'icon' : 'dot' ),
			);
		...
		public function ctl_attr_filter( $attr ) {
			...
			foreach ( $attr as $key => $values ) {
				if ( 'date-format' === $key || 'custom-date-format' === $key ) {
