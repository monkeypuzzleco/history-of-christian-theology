...
if ( ! class_exists( 'CTL_Loop_Helpers' ) ) {
	...
	class CTL_Loop_Helpers {
		...
		public function ctl_render( $index, $post ) {
			...
			$output .= '<!-- Timeline Content --><div  id="ctl-story-' . esc_attr( $post_id ) . '" class="' . esc_attr( implode( ' ', $classes ) ) . '"  data-story-index="' . esc_attr( $index ) . '">';
		...
		public function ctl_get_date( $post_id, $date_formats ) {
			...
			if ( $story_date ) {
				...
				if ( ! empty( $formatted_date ) ) {
					...
					$output .= '<div class="ctl-label-big story-date">' . esc_html( $formatted_date ) . '</div>';
					$output .= '</div>';
				}
			}
			 ...
			 * @param int $post_id Current Post ID.
			 */
		public function ctl_get_labels( $post_id ) {
			...
			if ( ! empty( $ctl_story_lbl ) || ! empty( $ctl_story_sub_lbl ) ) {
				...
				$output .= '<div class="ctl-label-big">' . wp_kses_post( $ctl_story_lbl ) . '</div>';
				$output .= '<div class="ctl-label-small">' . wp_kses_post( $ctl_story_sub_lbl ) . '</div>';
				$output .= '</div>';
			}
		...
		public function get_story_link( $post_id ) {
			...
			if ( 'design-7' === $attributes['designs'] ) {
				$story_link = '<a class="minimal_glightbox" data-popup-id="#ctl-popup-' . $post_id . '">';
			} else {
		...
		public function ctl_get_slideshow( $post_id ) {
			...
			foreach ( $slides as $index ) {
				...
				if ( wp_get_attachment_image_src( $index ) !== false && isset( $slide[0] ) ) {
					$image_gallery = '<a class="ctl_glightbox_gallery" data-gallery="ctl_glightbox_gallery[g_gallery-' . esc_attr( $post_id ) . ']" href="' . esc_attr( wp_get_attachment_image_src( $index, 'full' )[0] ) . '">';
					if ( is_array( $slide ) && $slide[0] ) {
						...
						$slides_html  .= '<div class="story-slide swiper-slide">' . $image_gallery . $image . '</a></div>';
					}
				}
			}
		...
		public function ctl_minimal_content() {
			...
			$output .= 'custom' === $attributes['based'] ? $this->ctl_get_labels( $post_id ) : $this->ctl_get_date( $post_id, $attributes['date-format'] );
			$output .= '</div>
