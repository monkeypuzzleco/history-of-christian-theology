     ...
     if(in_array($active_design,array("design-2","default","design-4","design-5","design-6","design-7"))){ 
        if($layout=="compact" && $attribute['compact-ele-pos']=="main-date" ){
                if($active_design=="design-7"){
                  ...
                  $popup_link_close='';
                  if($r_more=="yes"){
                    $popup_link_open='<a ref="minimal_glightbox" href="#ctl-'.esc_attr($post_id).'">';
                    $popup_link_close='</a>';
                  }
                  $ctl_html .='<h2 class="content-title"><div class="clt-cstm-lbl-f">'.$ctl_story_lbl.' <small class="clt-cstm-lbl-s">'.$ctl_story_lbl2.'</small></div><br/>'.$popup_link_open.esc_html(get_the_title()).$popup_link_close.'</h2>';
                }else{
                ...
                if($active_design=="design-6"){
                  ...
                  $popup_link_close='';
                  if($r_more=="yes"){
                    $popup_link_open='<a class="minimal_glightbox" href="#ctl-'.esc_attr($post_id).'">';
                    $popup_link_close='</a>';
                  }
                  $ctl_html .='<h2 class="content-title">'.$popup_link_open. esc_html(get_the_title()) .$popup_link_close.'</h2>';
                  // $ctl_html .='<h2 class="content-title">'.$popup_link_open. esc_html(get_the_title()) .$popup_link_close.'<br/><div class="clt-cstm-lbl-f">'.$ctl_story_lbl.' <small class="clt-cstm-lbl-s">'.$ctl_story_lbl2.'</small></div></h2>';
                }else{
      ...
      if($layout=="compact"){
         if($attribute['compact-ele-pos']=="main-title" ){
          if($active_design=="design-3" || $active_design=="design-6") {
            ...
            if($active_design=="design-3"||$active_design=="design-6") {
            $ctl_html .= '<h2 class="content-title-2">'.$slink_s. esc_html(get_the_title()) .$slink_e.'</h2>';
             }
           } 
