...
// stories content wrapper
  ...
  $ctl_html .= '<div   data-aos="' . esc_attr( $ctl_animation ) . '" class="timeline-meta"><div class="meta-details">';

  // if design 6  then enable icon
if ( $active_design == 'design-6' ) {
	if ( $icons == 'YES' ) {
		$icon      = ctl_post_icon( $post_id, $default_icon );
		$ctl_html .= '<div   data-aos="' . esc_attr( $ctl_animation ) . '" class="timeline-icon ' . esc_attr( $design ) . '-icon">
...
if ( in_array( $active_design, array( 'design-2', 'default', 'design-4', 'design-5', 'design-6' ) ) ) {
	if ( $active_design == 'design-6' ) {
				...
				$ctl_html .= '<h2 class="content-title">' . $slink_s . wp_kses_post(get_the_title()) . $slink_e . '</h2>';
	}
} elseif ( $active_design == 'design-7' ) {
	 $popup_link_open  = '';
	 $popup_link_close = '';
	if ( $r_more == 'yes' ) {
	 ...
	 $ctl_html .= '<h2 class="content-title">' . $popup_link_open . wp_kses_post( get_the_title() ) . $popup_link_close . '</h2>';
}
...
if ( $active_design == 'design-7' ) {
	...
	$ctl_html .= '<h2 class="popup-content-title">' . wp_kses_post( get_the_title() ) . '</h2>';
	...
	if ( $active_design == 'design-3' || $active_design == 'design-6' ) {
		$ctl_html .= '<h2 class="content-title-2">' . $slink_s . wp_kses_post( get_the_title() ) . $slink_e . '</h2>';
	}
