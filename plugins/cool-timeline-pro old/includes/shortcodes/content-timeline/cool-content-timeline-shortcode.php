...
if (!class_exists('CoolContentTimeline')) {
    ...
    class CoolContentTimeline
    {
           ...
           $this->assetsObj->clt_conditional_assets($tm_active_design,$ctl_animation,$layout,$type,$filters,$pagination,$attribute['year-navigation']);
            if ( $layout == 'horizontal') {
                ...
                if($pagination=="ajax_load_more" || $filters == 'yes'){
                    wp_localize_script( 'ctl-horizontal-load-more', "ct_load_more_$newUID",
      ...
      $alternate=0;
      $ctl_title_text='';
      $lineFilling       = $active_design =='design-5' && $layout == 'horizontal' ? ' data-line-filling="'.esc_attr($attribute['line-filling']).'"' : null;
      if(isset($ctl_options_arr['timeline_header']['title_text'])){
      $ctl_title_text=ctl_set_default_value($ctl_options_arr['timeline_header']['title_text'], 'Timeline');
      }
      $ctl_html_no_cont = '';
        $args = array();
        $cat_timeline = array();
      ...
      $itcls='';
      if($active_design == "default" || $active_design=="design-2" || $active_design=="design-3"  || $active_design=="design-4" || $active_design=="design-5" || $active_design=="design-6") {
      ...
      $sl_dir=is_rtl() ? "rtl":"";
        ...
        if ($layout == "horizontal") {
            ...
            $timeline_id=uniqid();
              ...
              $output .='<div class="clt_preloader" style="width: 100%; min-height: 300px; display: flex; justify-content: center; align-items: center;">
            ...
            $output .= '<div style="opacity:0" id="'.esc_attr($timeline_wrp_id).'" class="cool-timeline-horizontal  '.esc_attr($wrp_cls).' '.esc_attr($design_cls).'" date-slider="ctl-h-slider-'.esc_attr($timeline_id).'" data-nav="nav-slider-'.esc_attr($timeline_id).'" data-start-on="'.esc_attr($attribute['start-on']).'" data-autoplay="'.esc_attr($attribute['autoplay']).'" data-autoplay-speed="'.esc_attr($attribute['autoplay-speed']).'" data-items="'.esc_attr($items).'" data-rtl="'.$rtl.'" data-max-num-pages="'.esc_attr($ctl_loop->max_num_pages).'" data-wrapper="'.esc_attr($main_wrp_id).'" pagination="'.esc_attr($pagination).'" filters="'.$filters.'"'.$lineFilling.' >
        <div class="timeline-wrapper '.esc_attr($wrapper_cls).' '.esc_attr($itcls).'" >';
        
        // if($attribute['filters']=="yes"){
        //     $output.=ctl_categories_filters($timeline_id,$post_taxonomy,$select_cat=$post_category,$type="content-tm" ,$layout,$filter_categories);
        //  }
        if($hr_year_label=="show" && $year_label != ''){
            $output.='<div class="year-nav-wrp ctl-navigation-'.esc_attr($navigtion_position).'" data-id="'.esc_attr($timeline_id).'"><ul class="ctl_h_yearNav" id="year-slder-'.esc_attr($timeline_id).'">'.$year_label.'</ul></div>';
        }
            ...
            $output .= '<ul class="ctl_h_slides"  id="ctl-h-slider-'.esc_attr($timeline_id).'" data-filter="'.esc_attr($filters).'">';
            $output .=$ctl_html;
            $output .= '</ul></div>';
...
// var_dump($filters);
            ...
            id="'.esc_attr($main_wrp_id).'" class="'.implode(" ",$main_wrp_cls).'"  data-pagination="' .esc_attr($enable_navigation). '"  data-pagination-position="' .esc_attr($navigation_position) . '" data-line-filling="' .esc_attr($attribute['line-filling']). '">';
            if($attribute['filters']=="yes"){
                $type="content-tm";
                $output.=ctl_categories_filters($post_taxonomy,$select_cat=$post_category,$type,$layout,$filter_categories,$timeline_id);
            }
          ...
          $output.='<div class="clearfix"></div>';
            ...
            if($layout == 'default' || $layout == 'one-side'){
                $output.='<div class="cool_timeline_end"></div>';
            }
            $output.='<div class="ctl_center_line_filling"></div>';
            $output .= '</div> </div>  <!-- end
           ================================================== -->';
