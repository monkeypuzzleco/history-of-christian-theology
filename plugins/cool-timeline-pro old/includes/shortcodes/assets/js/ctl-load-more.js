	...
	class CtpLoadMore {
		...
		vrLoadMore = async () => {
			...
			try {
				...
				if (contentUpdate && response.data?.HTML) {
		...
		ctlBeforeContentLoad = (response) => {
			...
			if (this.newTimeline) {
				const childNodes = this.timelineContainer[0]?.childNodes;
			...
			this.timelineContainer
				.closest('.ctl-wrapper')
				.find('.ctl-category-container .ctl-loader-spinner')
				?.remove();
		...
		ctlAfterContentLoad = (response) => {
			...
			if (yearNavigation === 'show') {
				if (response.data?.HR_NAV_SLIDER) {
			...
			if (this.loadMoreBtn.length === 0) {
				const newPagination = jQuery(response.data?.newPagination);
		...
		getDataObject = () => {
			...
			const lastYear = this.timelineContainer
				.closest('.cool-timeline-wrapper')
				.find('.timeline-year.scrollable-section')
				.last()
		...
		updateYearNavigation = (HTML) => {
			...
			tempElement.find('.ctl-year').each((index, element) => {
				// timeline year label
		...
		// Aniket Extra feature for vertical layout start
