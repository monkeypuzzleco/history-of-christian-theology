...
function ctl_pro_block_callback( $attr ) {
	...
	if ($layout=="horizontal") {
		...
		show-posts="%s" date-format="%s" icons="%s" 
		 designs="%s" category="%s" story-content="%s" based="%s" line-filling="%s"
		 autoplay="%s" start-on="%s" items="%s" pagination="%s" year-navigation="%s" navigation-position="%s" year-label="%s" order="%s" autoplay-speed="%s"
		 ]';
	 $shortcode= sprintf( $shortcode_string, $layout, $skin, 
		$postperpage,$dateformat,$icons,
		$designs,$category,$storycontent,$based,$lineFilling,$autoplay
		,$starton,$items,$pagination,$hrNavigation,$hrNavigationPosition,$hrYearLabel,$order,$autoplayspeed);
		return $shortcode;
	}else{
		$shortcode_string = '[cool-timeline layout="%s" skin="%s"
		show-posts="%s" date-format="%s" icons="%s" animations="%s"
		 designs="%s" category="%s" story-content="%s" based="%s"
		 compact-ele-pos="%s" pagination="%s" filters="%s" filter-categories="%s" line-filling="%s" order="%s"]';
		 $shortcode= sprintf( $shortcode_string, $layout, $skin, 
		$postperpage,$dateformat,$icons,$animation,
		$designs,$category,$storycontent,$based,
		$compactelepos,$pagination,$filters,$filtercategories,$lineFilling,$order);
		return $shortcode;
	}
}
...
function ctl_content_tm_block_callback( $attr ) {
	...
	if ($layout=="horizontal") {
		...
		show-posts="%s" date-format="%s" icons="%s" 
		 designs="%s" category="%s" story-content="%s" line-filling="%s"
		 autoplay="%s" start-on="%s" items="%s" pagination="%s" year-navigation="%s" navigation-position="%s" year-label="%s" order="%s" autoplay-speed="%s" 
		 post-type="%s" post-category="%s" tags="%s" taxonomy="%s"]';
		  $shortcode= sprintf( $shortcode_string, $layout, $skin, 
		$postperpage,$dateformat,$icons,
		$designs,$category,$storycontent,$lineFilling,$autoplay
		,$starton,$items,$pagination,$hrNavigation,$hrNavigationPosition,$hrYearLabel,$order,$autoplayspeed,
