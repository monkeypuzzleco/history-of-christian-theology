...
if (!class_exists('CoolTimelinePosttype')) {
    ...
    class CoolTimelinePosttype {
        ...
        function cooltimeline_custom_post_type() {
            ...
            $args = array(
                'label' => __('cool-timeline', 'cool-timeline2'),
                ...
                'rewrite' =>$slug_arr ,
                'menu_icon'=>CTP_PLUGIN_URL.'assets/images/timeline-icon-222.png',
            );
        ...
        function ctl_taxonomy() {
            ...
            $labels = array(
                'name' => _x('Categories', 'Taxonomy General Name', 'cool-timeline2'),
                'singular_name' => _x('Category', 'Taxonomy Singular Name', 'cool-timeline2'),
                'menu_name' => __('Categories', 'cool-timeline2'),
                'all_items' => __('All Items', 'cool-timeline2'),
                'parent_item' => __('Parent Item', 'cool-timeline2'),
                'parent_item_colon' => __('Parent Item:', 'cool-timeline2'),
                'new_item_name' => __('New Item Name', 'cool-timeline2'),
                'add_new_item' => __('Add New Item', 'cool-timeline2'),
                'edit_item' => __('Edit Item', 'cool-timeline2'),
                'update_item' => __('Update Item', 'cool-timeline2'),
                'view_item' => __('View Item', 'cool-timeline2'),
                'separate_items_with_commas' => __('Separate items with commas', 'cool-timeline2'),
                'add_or_remove_items' => __('Add or remove items', 'cool-timeline2'),
                'choose_from_most_used' => __('Choose from the most used', 'cool-timeline2'),
                'popular_items' => __('Popular Items', 'cool-timeline2'),
                'search_items' => __('Search Items', 'cool-timeline2'),
                'not_found' => __('Not Found', 'cool-timeline2'),
                'no_terms' => __('No items', 'cool-timeline2'),
                'items_list' => __('Items list', 'cool-timeline2'),
                'items_list_navigation' => __('Items list navigation', 'cool-timeline2'),
            );
            $args = array(
                'labels' => $labels,
                'hierarchical' => true,
        ...
        function custom_columns($column, $post_id) {
         global   $post ;
          ...
          $story_based_on = isset($ctl_story_type['story_based_on'])?$ctl_story_type['story_based_on']:'default';
            ...
            switch ($column) {
                ...
                case "order":
                  if($story_based_on=="custom"){
                    $ctl_story_order = isset($ctl_story_type['ctl_story_order'])?$ctl_story_type['ctl_story_order']:''; 
                    
                     echo'<div class="quick-order-update"><input size="5" value="'.$ctl_story_order.'" type="text" data-id="#pld-'.$post_id.'" name="clt_story_order" class="custom_story_order" data-post-id="'.$post_id.'"><img style="width:16px;display:none;margin-left:5px;" class="od_preloader" id="pld-'.$post_id.'" src="'.CTP_PLUGIN_URL.'assets/images/order-preloader.gif"></div>';
                    }
                  break;
                ...
                case 'category' :
                    ...
                    break;
                    default:
                    echo "<p>".esc_html_e( '', 'cool-timeline' )."</p>";
                }
        }
    ...
    function ctl_change_story_order() {
        ...
        $p_id = intval( $_POST['post_id'] );
          //if($new_order && $p_id){
           ...
           $ctl_story_type = get_post_meta($p_id, 'story_type', true);
           $ctl_story_type['ctl_story_order'] = $new_order;                      
           $rs= update_post_meta($p_id, 'story_type', $ctl_story_type);
           $ctl_story_order = isset($ctl_story_type['ctl_story_order'])?$ctl_story_type['ctl_story_order']:''; 
